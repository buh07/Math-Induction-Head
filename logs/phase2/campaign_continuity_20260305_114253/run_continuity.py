...
def _ps_rows() -> List[Tuple[int, str]]:
    proc = subprocess.run(['ps', '-eo', 'pid=', 'args='], cwd=ROOT, capture_output=True, text=True)
...
def _pid_for_run_dir(run_dir: Path) -> Optional[int]:
    ...
    for pid, args in _ps_rows():
        if 'run_operator_bottleneck_suite.py' in args and '--output-root' in args and needle in args:
            return pid
    return None


def _any_inflight_current_run() -> List[int]:
    needle = str(CURRENT_RUN_ROOT)
    pids: List[int] = []
    for pid, args in _ps_rows():
        if 'run_operator_bottleneck_suite.py' in args and '--output-root' in args and needle in args:
            pids.append(pid)
...
def _run_phase2_model_sharded(model_alias: str) -> Path:
    ...
    while running:
        ...
        for op, (proc, gpu) in running.items():
            rc = proc.poll()
            if rc is None:
                continue
        ...
        while queue and len(running) < len(gpu_slots):
            op = queue.pop(0)
            used = {gpu for (_p, gpu) in running.values()}
...
def main() -> int:
    ...
    for model_alias in PHASE2_MODELS:
        ...
        if unit.get('reuse_eligible'):
            ...
            merged = _run_phase2_model_sharded(model_alias)
            canonical_phase2[model_alias] = {
                'status': 'completed',
            ...
            _log(f'rerun-complete phase2 model={model_alias} merged={merged}')
